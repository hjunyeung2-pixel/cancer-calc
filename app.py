...
def save_pdf(customer_name, company_data, yearly_events, yearly_payouts, total_amount):
    ...
    # 1장 - 가입담보 + 치료내역
    # -----------------------------
    story.append(Paragraph("📑 암주요치료비 보장 통합 제안서", styles['CenterTitle']))
    story.append(Spacer(1, 10))
    story.append(Paragraph(
        f"고객명: <b>{customer_name}</b><br/>"
        "본 제안서는 고객님의 건강을 든든히 지켜드릴 맞춤형 보장을 정리한 자료입니다.",
        styles['KoreanBody']
    ))
    ...
    # 회사별 가입 담보
    if company_data:
        story.append(Paragraph("1. 회사별 가입 담보 정리", styles['SectionHeader']))
        for comp, rows in company_data.items():
            if not rows:
                continue
            table_data = [["담보명", "가입금액(만원)"]] + rows
            ...
            story.append(Paragraph(f"◼ {comp}", styles['KoreanBold']))
            story.append(table)
            story.append(Spacer(1, 12))

    # 연도별 치료내역
    if yearly_events:
        story.append(Paragraph("2. 연도별 치료 내역", styles['SectionHeader']))
        table_data = [["연도", "치료 내역"]]
        for year, evs in yearly_events.items():
            if not evs:
                continue
            table_data.append([f"{year}년차", " / ".join(evs)])
    ...
    # -----------------------------
    if yearly_payouts:
        story.append(Paragraph("3. 연도별 지급 내역", styles['SectionHeader']))
        ...
        for year, pays in yearly_payouts.items():
            ...
            year_total = 0
            year_start = row_idx
            for (treaty, amt) in pays:   # (특약명, 금액)
                if amt <= 0:
                    continue
                table_data.append([f"{year}년차", treaty, f"{amt:,}"])
                year_total += amt
                row_idx += 1

            # 연도 병합 (첫 행만 표시)
            if row_idx - year_start > 1:
                for r in range(year_start+1, row_idx):
                    table_data[r][0] = ""

            # 합계 행
            table_data.append(["▶ 합계", "", f"{year_total:,}"])
            row_idx += 1
        ...
        story.append(Paragraph(
            f"💡 <b>총 {grand_total:,} 만원의 보험금</b>이 지급될 수 있습니다.<br/>"
...
MAPPING_SAMSUNG = {
    ...
    "항암약물": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
             ("항암약물치료특약", 1, 1.0)],
    "항암호르몬치료제": [("암직접치료보장", 1, 1.0)], 
    "중입자": [("암직접치료보장", 1, 1.0), ("항암방사선치료특약", 1, 1.0), 
            ("항암중입자방사선치료특약", 1, 1.0)],
    "세기조절": [("암직접치료보장", 1, 1.0), ("항암방사선치료특약", 1, 1.0),
             ("프리미엄암직접치료보장", 1, 1.0)],
    "양성자": [("암직접치료보장", 1, 1.0), ("항암방사선치료특약", 1, 1.0), 
            ("프리미엄암직접치료보장", 1, 1.0)],
    "정위적": [("암직접치료보장", 1, 1.0), ("항암방사선치료특약", 1, 1.0), 
            ("프리미엄암직접치료보장", 1, 1.0)],
    "로봇수술": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
             ("프리미엄암직접치료보장", 1, 1.0), ("프리미엄클래스암특정치료보장", 1, 1.0)],
    "표적(급여)": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
               ("프리미엄암직접치료보장", 1, 1.0)],
    "표적(비급여)": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
                ("프리미엄암직접치료보장", 2, 1.0), ("프리미엄클래스암특정치료보장", 1, 1.0)],
    "면역(급여)": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
               ("프리미엄암직접치료보장", 2, 1.0)],
    "면역(비급여)": [("암주요치료보장", 1, 1.0), ("암직접치료보장", 1, 1.0), 
    ...
    for ev in events:
        if ev and ev in MAPPING:
            for (treaty, cnt, rate) in MAPPING[ev]:
                if treaty in ["비급여 암 주요치료비 II", "암직접치료보장"]:
...
for year in range(years[0], years[1] + 1):
    ...
    total_s, details_s, usage_counter_samsung = calc_payments(
        events, insurance_amounts_samsung, usage_counter_samsung, MAPPING_SAMSUNG, MAX_USAGE_SAMSUNG, year
    )
    total_k, details_k, usage_counter_kb = calc_payments(
        events, insurance_amounts_kb, usage_counter_kb, MAPPING_KB, MAX_USAGE_KB, year
    )

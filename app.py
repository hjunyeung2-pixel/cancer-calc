...
REGULAR_PATH = os.path.join(FONT_DIR, "NanumGothic.ttf")
...
# (옵션) 이모지/미지원 글리프 제거 함수
_EMOJI_RE = re.compile(
    r"[\U00010000-\U0010FFFF]"  # BMP 밖의 문자(대부분 이모지)
)
...
def save_pdf(customer_name, company_data, yearly_events, yearly_payouts, total_amount):
    ...
    intro = f"고객명: <b>{safe_text(customer_name)}</b><br/>" \
            "본 제안서는 고객님의 건강을 든든히 지켜드릴 맞춤형 보장을 정리한 자료입니다."
    story.append(Paragraph(intro, styles['KoreanBody']))
    ...
    if yearly_payouts:
        ...
        for year, pays in yearly_payouts.items():
            ...
            # 항목이 없으면 대시 한 줄(병합 불필요)
            if item_count == 0:
                table_data.append([f"{year}년차", "-", "0"])
                row_idx += 1
                item_count = 1

            # 같은 연도 셀 병합: 첫 행 ~ 마지막 아이템 행 (합계 제외)
            if item_count >= 2:
                spans.append(('SPAN', (0, visible_rows_start), (0, visible_rows_start + item_count - 1)))
